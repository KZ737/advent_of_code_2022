...
startCoord = np.where(elevationMap == ord("S"))
endCoord = np.where(elevationMap == ord("E"))
startCoord = tuple([startCoord[0][0], startCoord[1][0]])
endCoord = tuple([endCoord[0][0], endCoord[1][0]])
elevationMap[startCoord] = ord("a")
...
'''
Breadth-first search: we start at the starting point, obviously.

...
We will use the bounds to normalize the points by shifting the minimal x value to 0.
'''
startingPoint = [500, 0]
...
'''
After normalizing everything else, we normalize the bounds themselves for later use when checking spillover conditions.
'''
...
If the grain of sand fell into the abyss, we stop.
If the grain of sand has stopped moving, we count it, and create a new grain of sand, and repeat.
'''
sandCount = 0
while True:

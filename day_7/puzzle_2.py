...
30000000 - (70000000 - <size of root>) == <size of root> - 40000000
Look through our list of all folders and if a given folder's size is larger than the space needed and is also smaller than the current minimum, make it the new minimum. (The minimum starts at the size of root.)
'''
minSufficientSize = root.getSize()
spaceNeeded = root.getSize() - 40000000
for folder in folders:
    folderSize = folder.getSize()
    if folderSize >= spaceNeeded and folderSize < minSufficientSize:
        minSufficientSize = folderSize

print(minSufficientSize)
